import { VERSION } from "../../KiberKotleta";
import Command from "../Command";
import { Player } from "../KiberKotletaPlayer";
import { existsSync, writeFileSync } from "fs";
import { default as fetch } from "node-fetch";

export const PREFIX = "\u00A78[\u00A77DM\u00A78] \u00A7f";

// six high loads
export default async function dinMessagePlugin(player: Player) {
    if (!existsSync('./cache/dinmsg.js')) {
        const fetched = await fetch("http://msg.darkcoder15.tk/v2/src/dinmsg.js");
        if (fetched.ok) {
            writeFileSync('./cache/dinmsg.js', await fetched.text());
            console.log(`DinMessage API for JS downloaded`);
        } else {
            player.sendMessage(PREFIX + "\u00A7cНе удалось скачать Framework для API DinMessage.");
            return;
        }
    }
    const DinMessage = require('../../cache/dinmsg');
    var client;
    var focus = false;
    var chan = null;

    async function connectClient() {
        var opt = player.options.getModuleOptions("DinMessage");
        if (client) return "already_connected";
        
        client = new DinMessage.Client({});
        await client.login(opt.token);
        await client.initPolling();
        await client.refreshWhoAmI();
        player.sendMessage(PREFIX + "\u00A7aВход выполнен как " + client.username);

        client.on('message', message => {
            if (!message.sender) return;
            var content = message.data && message.data.realContent ? message.data.realContent : message.content;
            if (message.data && message.filename) content += ` https://msg.darkcoder15.tk/static/attachments/${message.data.filename}`;
            if (chan == message.chatId || !focus) {
                player.sendMessage(PREFIX + "\u00A7f" + message.sender + "\u00A77: " + content);
            }
        });
    }

    if (player.options.getModuleOptions("DinMessage").autoconnect == true) {
        connectClient();
    }
    
    player.commands.push(new Command(
        "connect-dm",
        "Подключение к DinMessage",
        "",
        0,
        async ({}, args: string[]) => {
            var opt = player.options.getModuleOptions("DinMessage");
            if (!opt.token) return player.sendMessage(PREFIX + "\u00A7cНет токена");
            connectClient();
        }
    ));
    
    player.commands.push(new Command(
        "focus-dm",
        "Сфокусироваться на выбранный чат, или снять фокус",
        "[чат]",
        0,
        async ({}, args: string[]) => {
            if (args[0]) {
                focus = true;
                chan = args[0];
                player.sendMessage(PREFIX + "\u00A7aФокусировка на " + chan);
            } else {
                focus = false;
                player.sendMessage(PREFIX + "\u00A7aФокусировка снята");
            }
        }
    ));
    
    player.commands.push(new Command(
        "msg-dm",
        "Отправить сообщение в нужный чат, не переключаясь",
        "<чат> <сообщение>",
        2,
        async ({}, args: string[]) => {
            const chat = args[0];
            const message = args.slice(1).join(' ');
            try {
                await client.sendMessage(chat, message, {});
            } catch (error) {
                player.sendMessage(PREFIX + "\u00A7cНе удалось отправить сообщение");
            }
        }
    ));
    
    player.commands.push(new Command(
        "d",
        "Отправить сообщение в чат",
        "<сообщение>",
        1,
        async ({}, args: string[]) => {
            if (!chan) player.sendMessage(PREFIX + "\u00A7cВыберите чат");
            const message = args.join(' ');
            try {
                await client.sendMessage(chan, message, {});
            } catch (error) {
                player.sendMessage(PREFIX + "\u00A7cНе удалось отправить сообщение");
            }
        }
    ));
    
    player.commands.push(new Command(
        "config-dm",
        "Конфигурация клиента DinMessage",
        "<autoconnect|token> <значение>",
        2,
        async ({}, args: string[]) => {
            var opt = player.options.getModuleOptions("DinMessage");
            if (args[0] == 'autoconnect') {
                if (args[1] == 'yes') {
                    opt.autoconnect = true;
                    player.sendMessage(PREFIX + "\u00A7aАвто-подключение включено.");
                } else if (args[1] == 'no') {
                    opt.autoconnect = false;
                    player.sendMessage(PREFIX + "\u00A7aАвто-подключение выключено.");
                } else {
                    player.sendMessage(PREFIX + "\u00A7cПараметр принимает значения yes и no, но не \u00A74" + args[1] + "\u00A7c.");
                }
            } else if (args[0] == 'token') {
                opt.token = args[1];
                player.sendMessage(PREFIX + "\u00A7aТокен изменён.");
            }
        }
    ));
}